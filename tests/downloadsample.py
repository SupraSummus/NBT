    ...
    for path in checksums.keys():
        try:
            ...
            if e.errno == 2:
                ...
                logger.error(
                    'Checksum verificiation of %s failed: errno %d: %s' %
...
def install(url=URL, workdir=workdir, checksums=checksums):
    ...
    nchecksums = dict([
        (os.path.join(workdir, os.path.normpath(path)), checksums[path])
        for path in posixpaths if checksums[path] is not None
    ])
        ...
        if not os.path.exists(tar_file):
            ...
            try:
                ...
                logger.error(
                    'Download %s failed: %s' % (
                        url,
                        e.reason.strerror if hasattr(e.reason, "strerror") else e.reason,
                    )
                )
                ...
                logger.error(
                    'Download %s failed: HTTP Error %d: %s' % (
                        url, e.code,
